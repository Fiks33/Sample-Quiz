package com.fik.android.quiz.adapter;
...
import com.fik.android.quiz.R;
import com.fik.android.quiz.data.DataQuestion;

import java.util.List;

/**
 * Created by Mochamad Taufik on 10-Dec-18.
...
public class AdapterRecyclerQuestion extends RecyclerView.Adapter<AdapterRecyclerQuestion.ViewHolder> {
    ...
    public AdapterRecyclerQuestion(List<DataQuestion> items) {
        this.items = items;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_row_question, parent, false);
        return new ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        DataQuestion item = items.get(position);
